...
export function buildMalibu(ctx, M, handoff, lod) {
  ...
  // stone floor inside the glass line
  {
    ...
    // underneath achieves nothing on its own: the player stands on the finish floor over
    // the hole and reports, correctly, that there is glass across the stairwell.
    // ShapeGeometry has no bevel, so a hole in the Shape is safe here — unlike the slab,
    // where the same trick degenerates the extrusion.
    {
      const hole = new THREE.Path();
      const seg = 40;
      for (let i = 0; i <= seg; i++) {
        const a = (i / seg) * Math.PI * 2;
        const hx = STAIR.x + Math.cos(a) * STAIR.r;
        const hy = STAIR.z + Math.sin(a) * STAIR.r;
