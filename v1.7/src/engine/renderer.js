...
export function createRenderer(host) {
  ...
  // Shadows are not redrawn every frame.
  //
  // Measured in the workshop at 1280x720: shadows cost 61 ms of a 76 ms frame — 80% of
  // the whole draw. The reason is volume, not resolution: 2735 meshes cast, and every
  // shadow-casting light re-renders all of them into its map on every single frame.
  //
  // This world is almost entirely static: the house, the cliff, the town and the props do
  // not move, and the sun barely does. Refreshing the maps every 4th frame is invisible
  // in motion and cuts the cost to a quarter. loop.js flips needsUpdate on the beat.
