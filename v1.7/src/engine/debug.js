...
export function installDebug(ctx, loop) {
  ...
  const MZ = {
    ...
    /** Per-module update cost. profile(n) runs n ticks and returns ms per module. */
    profile(n = 120) {
      loop.profile = {};
      for (let i = 0; i < n; i++) loop.step(1 / 120);
      const raw = loop.profile;
      loop.profile = null;
      const rows = Object.entries(raw)
        .map(([id, r]) => ({ id, avgMs: +(r.total / r.n).toFixed(3), maxMs: +r.max.toFixed(2) }))
        .sort((a, b) => b.avgMs - a.avgMs);
      const total = rows.reduce((s2, r) => s2 + r.avgMs, 0);
      return { totalPerTickMs: +total.toFixed(2), rows };
    },
