...
export function buildMalibu(ctx, M, handoff, lod) {
  ...
  const DRUM_H = L2 + 1.2 - (L0 - SLAB);
  const DRUM_MID = (L0 - SLAB + L2 + 1.2) / 2;
  // A doorway facing the living room, at ground-floor height. The angle is atan2(x, z)
  // measured from the drum centre toward where the player actually walks in from; a
  // doorway on the far side is the same as no doorway. Without it the drum is a sealed
  // collider and the stair inside is both unreachable and invisible.
  const drumGeo = punchDoor(
    new THREE.CylinderGeometry(6.2, 6.2, DRUM_H, 40, 1, true),
    { a0: -1.39 - 0.85, a1: -1.39 + 0.85, y0: L0 - DRUM_MID - 0.1, y1: L0 + 2.7 - DRUM_MID }
  );
  const drum = new THREE.Mesh(drumGeo, M.slab);
